...
export default function HomePage() {
  ...
  // ✅ FIXED SEARCH HANDLER (URL BASED)
  const handleSearch = () => {
  ...
  const handleChipClick = (chipQuery) => {
    setQuery(chipQuery)
    navigate(`/results?q=${encodeURIComponent(chipQuery)}`)
  }
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[
            "👩 2BHK ECIL under ₹20k, girl-friendly",
            "💼 3BHK Gachibowli under ₹30k, bachelors ok",
            "🏠 Family home Kompally under ₹18k"
          ].map(chip => (
            <button
              key={chip}
              onClick={() => handleChipClick(chip.substring(2))}
              style={{
                background: 'white',
