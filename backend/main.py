...
AREA_NEARBY = {
    ...
    "ameerpet": ["sr nagar", "begumpet"],
    "madhapur": ["gachibowli", "kondapur"],
    "ecil": ["kapra", "as rao nagar", "malkajgiri"],
}

def quick_enrich(prop):
    import random

    return {
        **prop,
        "score": round(random.uniform(6.5, 9.5), 1),
        "water_score": round(random.uniform(6, 9), 1),
        "safety_score": round(random.uniform(6, 9), 1),
        "transport_score": round(random.uniform(6, 9), 1),
        "schools_score": round(random.uniform(6, 9), 1),
        "it_hub_score": round(random.uniform(6, 9), 1),
        "nearby_amenities": ["School", "Hospital", "Metro", "Market"]
    }
...
def extract_requirements(query: str):
    ...
    # ✅ Budget (₹20k → 20000)
    rent_match = re.search(r'under\s*₹?\s*(\d+)', q)
    if rent_match:
        max_rent = int(rent_match.group(1)) * 1000

    # ✅ Areas (add more if needed)
    areas = ["ecil", "ameerpet", "kukatpally", "gachibowli", "madhapur"]
    for a in areas:
        if a in q:
            preferred_areas.append(a)

    return bhk, max_rent, preferred_areas, girl_friendly
...
async def enrich_property(prop: dict, requirements: dict) -> dict:
    ...
    # ✅ Scores
    water_score, weather_result = await asyncio.gather(
    ...
    if social_context.get("is_girl"):
        social_bonus += (social_scores.get("Girl Friendly", 6.0) - 6.0) * 2

    wants_families = social_context.get("wants_families_nearby")

    if wants_families is True:
        social_bonus += (social_scores.get("Family Score", 7.0) - 5.0)
        if prop.get("society_type") == "family":
            social_bonus += 5

    elif wants_families is False:
        social_bonus += (social_scores.get("Bachelor Score", 6.0) - 5.0)
        if prop.get("society_type") == "bachelor" or prop.get("bachelor_friendly"):
            social_bonus += 5
    ...
    if total_w > 0:
        ...
        final_score = int(base * 7 + price_score * 2 + social_bonus)
    else:
        final_score = 60
    ...
    # ✅ Match reason
    if social_context.get("is_girl"):
        match_reason = f"Safe {prop['bedrooms']}BHK in {prop['area']} suitable for women."
    elif social_context.get("is_bachelor"):
        match_reason = f"Bachelor-friendly {prop['bedrooms']}BHK in {prop['area']} within budget."
    else:
        match_reason = f"Well-located {prop['bedrooms']}BHK in {prop['area']} within ₹{prop['rent_monthly']}."

    # ✅ FIXED BADGES (INSIDE FUNCTION ✅)
    badges = []

    if prop["rent_monthly"] < max_rent * 0.6:
        badges.append("💰 Budget Friendly")

    if prop.get("bedrooms") == requirements.get("min_bedrooms"):
        badges.append("⭐ Exact Match")

    if score_breakdown["Safety"] > 8:
        badges.append("🛡️ Safe Area")

    if score_breakdown["Water Supply"] > 8:
        badges.append("💧 Good Water")
    ...
    return {
        **prop,
        ...
        "tags": tags,
        "badges": badges   # ⭐ IMPORTANT (frontend uses this)
    }
...
async def search_properties(request: SearchRequest):
    ...
    max_rent = requirements.get("max_rent", 50000)
    requested_bhk = requirements.get("min_bedrooms")
    preferred_areas = [a.lower() for a in requirements.get("preferred_areas", [])]
    ...
    for prop in ALL_PROPERTIES:
        ...
        rent = prop.get("rent_monthly", 0)

        if preferred_areas and area not in preferred_areas:
            continue

        if rent > max_rent * 1.2:
            continue

        base_results.append(prop)
    ...
    if requested_bhk:
        exact = [p for p in base_results if p["bedrooms"] == requested_bhk]
        higher = [p for p in base_results if p["bedrooms"] > requested_bhk]
        lower = [p for p in base_results if p["bedrooms"] < requested_bhk]

        # ⭐ IMPORTANT: exact FIRST
        final_results = exact + higher + lower
    else:
    ...
    # ✅ STEP 3: NEARBY AREAS
    # =========================
    if len(final_results) < 5 and preferred_areas:

        nearby_areas = []
        for area in preferred_areas:
            nearby_areas.extend(AREA_NEARBY.get(area, []))
        ...
        for prop in ALL_PROPERTIES:
            ...
            nearby_results.append(prop)

        # ✅ Apply BHK logic to nearby ALSO
        if requested_bhk:
            nearby_results = sorted(
                nearby_results,
                key=lambda x: (
                    abs(x["bedrooms"] - requested_bhk),
                    x["bedrooms"] < requested_bhk
                )
            )

        final_results.extend(nearby_results)

    # =========================
    # ✅ STEP 4: LIMIT
    # =========================
    filtered = final_results[:12]
    ...
    try:
        results = await asyncio.gather(
            *[enrich_property(p, requirements) for p in filtered]
        )
    ...
    print("REQUESTED BHK:", requested_bhk)
